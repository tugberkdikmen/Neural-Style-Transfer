...
style_img = image_loader("./test images/d.jpg")
content_img = image_loader("./test images/n.jpg")
...
load_path = "saved/trained_model1.pth"
cnn = models.vgg19(pretrained=False)  # Initialize the same model architecture
# Replace the last fully connected layer for your number of classes
num_classes = 2  # Update this number to match your desired number of classes
cnn.classifier[6] = nn.Linear(4096, num_classes)
cnn.load_state_dict(torch.load(load_path))
...
def get_style_model_and_losses(cnn, normalization_mean, normalization_std,
    ...
    '''
    i = 0  # increment every time we see a conv
    for layer in cnn.features.children():  # Iterate over the features part of the model
        if isinstance(layer, nn.Conv2d):
            i += 1
            name = 'conv_{}'.format(i)
        elif isinstance(layer, nn.ReLU):
        ...
        # Use the layer and name as required in your Neural Style Transfer code
        # ...
